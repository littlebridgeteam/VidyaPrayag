...
@Composable
fun AcademicYearManagementPremium(
    onBack: () -> Unit,
    ...
    Column(
        ...
        VBackHeader(title = "Academic Year Management", onBack = onBack)

        VStateHostPremium(
            loading = state.isLoading,
            error = state.errorMessage,
            isEmpty = state.years.isEmpty() && !state.isLoading,
            ...
            Column(
                ...
                val active = state.activeYear
                VGradientHeroPremium(
                    title = active?.name ?: "No Active Year",
                    subtitle = active?.let { "${it.startDate} to ${it.endDate}" } ?: "Create or activate a year",
                    stats = listOf(
                        HeroStatPremium("${state.years.size}", "Total"),
                        HeroStatPremium("${state.years.count { it.isActive }}", "Active"),
                        HeroStatPremium("${state.historicalYears.size}", "Archived"),
                    ),
                    onClick = {},
                ...
                Column(Modifier.padding(horizontal = 20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    Text("All Years", style = VTypography.SectionHeader.copy(color = VColors.OnSurface))
                    state.years.forEach { y ->
                        VListTilePremium(
                            title = y.name,
                            subtitle = "${y.startDate} to ${y.endDate}",
                            onClick = { viewModel.activate(y.id) },
                            leadingIcon = VIcons.Calendar,
                            trailingText = y.status,
                        )
                    }
                }
            }

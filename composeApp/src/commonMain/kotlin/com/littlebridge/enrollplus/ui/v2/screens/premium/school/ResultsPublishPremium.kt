...
fun ResultsPublishPremium(
    ...
    Column(
        ...
        VStateHostPremium(
            ...
            isEmpty = state.students.isEmpty() && !state.isLoading,
            emptyTitle = "No results data",
            ...
            Column(
                ...
                VGradientHeroPremium(
                    title = "Exam Results",
                    subtitle = "Class Average: ${state.classAverage}",
                    stats = listOf(
                        HeroStatPremium(state.classAverage, "Average"),
                        HeroStatPremium(state.averageTrend, "Trend"),
                        HeroStatPremium("${state.exceedingCount}", "Exceeding"),
                    ),
                    onClick = {},
                ...
                Column(Modifier.padding(horizontal = 20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    Text("Summary", style = VTypography.SectionHeader.copy(color = VColors.OnSurface))
                    VStatCardPremium(value = "${state.exceedingCount}", label = "Exceeding", onClick = {}, icon = VIcons.TrendingUp)
                    VStatCardPremium(value = "${state.meetingCount}", label = "Meeting", onClick = {}, icon = VIcons.Check)
                    VStatCardPremium(value = "${state.belowCount}", label = "Below", onClick = {}, icon = VIcons.AlertCircle)

                    Text("Students", style = VTypography.SectionHeader.copy(color = VColors.OnSurface))
                    state.students.forEach { s ->
                        VListTilePremium(
                            title = s.name,
                            subtitle = "Attendance: ${s.attendance} | Score: ${s.score}",
                            onClick = {},
                            leadingIcon = VIcons.Users,
                            trailingText = s.status,
                        )
                    }
                }
            }
